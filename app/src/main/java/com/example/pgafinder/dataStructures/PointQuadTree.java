...
/*
 * Copyright 2013 Google Inc./* ww w . ja v a2  s.  co
...
public class PointQuadTree<T extends PointQuadTree.Item> {
    ...
    public void add(T item) {
        myPoint point = item.getPoint();
        if (this.mBounds.contains(point.x, point.y)) {
            insert(point.x, point.y, point.SI, item);
        }
    }

    private void insert(double x, double y,double SI, T item) {
        if (this.mChildren != null) {
            if (y < mBounds.midY) {
                if (x < mBounds.midX) { // top left
                    mChildren.get(0).insert(x, y, SI, item);
                } else { // top right
                    mChildren.get(1).insert(x, y, SI, item);
                }
            } else {
                if (x < mBounds.midX) { // bottom left
                    mChildren.get(2).insert(x, y, SI, item);
                } else {
                    mChildren.get(3).insert(x, y, SI, item);
                }
            }
            return;
        }
    ...
    private void split() {
        ...
        for (T item : items) {
            ...
            insert(item.getPoint().x, item.getPoint().y, item.getPoint().SI, item);
        }
    }
